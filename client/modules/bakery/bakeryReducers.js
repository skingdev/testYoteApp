...
// import bakery actions
import * as Actions from './bakeryActions';
 ...
 *
 * NOTE: this is never called directly. Only by parent 'bakery' reducer (defined
 * below) when dealing with a LIST action
 */
function bakeryList(state = {
  /**
   ...
   * then recursively return an _additional_ bakeryList reducer.
   *
   * Otherwise, return the actual bakery lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional bakeryList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: bakeryList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_BAKERY_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_BAKERY_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_BAKERY_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
      ...
      case Actions.ADD_BAKERY_TO_LIST:
      ...
      case Actions.REMOVE_BAKERY_FROM_LIST:
        idArray = [...state.items]
      ...
      case Actions.SET_BAKERY_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_BAKERY_PAGINATION: {
        return {
          ...state
...
/**
 * Primary bakery reducer -
 *
 * This is the single source of truth for all things 'bakery' related within the
 ...
 */
function bakery(state = {

  /**
   * "defaultItem" defines fields for a _new_ bakery
   * any component that creates a new bakery object should store a copy of this
   * in its state
   */
  defaultItem: {
    name: ''
  ...
  /**
   * "byId" is an object map of all bakery items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a bakery, the single bakery
  ...
  /**
   * "lists" corresponds to individual instances of the bakeryList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
    /**
     * SINGLE BAKERY ACTIONS
     */
    case Actions.REQUEST_SINGLE_BAKERY: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_BAKERY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_BAKERY_TO_MAP: {
      // add this bakery to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_BAKERY: {
    ...
    case Actions.REQUEST_CREATE_BAKERY: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_BAKERY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_BAKERY: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_BAKERY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_BAKERY: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_DELETE_BAKERY: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_BAKERY: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_BAKERY_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: bakeryList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
