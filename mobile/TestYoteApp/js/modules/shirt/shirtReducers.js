...
// import shirt actions
import * as Actions from './shirtActions';
 ...
 *
 * NOTE: this is never called directly. Only by parent 'shirt' reducer (defined
 * below) when dealing with a LIST action
 */
function shirtList(state = {
  /**
   ...
   * then recursively return an _additional_ shirtList reducer.
   *
   * Otherwise, return the actual shirt lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional shirtList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: shirtList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_SHIRT_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_SHIRT_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_SHIRT_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
      ...
      case Actions.ADD_SHIRT_TO_LIST:
        ...
        case Actions.SET_SELECTED_SHIRT: {
      ...
      case Actions.REMOVE_SHIRT_FROM_LIST:
        idArray = [...state.items]
      ...
      case Actions.SET_SHIRT_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_SHIRT_PAGINATION: {
        return {
          ...state
...
/**
 * Primary Shirt reducer -
 *
 * This is the single source of truth for all things 'shirt' related within the
...
function shirt(state = {
  ...
  /**
   * "defaultItem" defines fields for a _new_ shirt
   * any component that creates a new shirt object should store a copy of this
   * in its state
   */
  defaultItem: {
    title: ''
  ...
  /**
   * "byId" is an object map of all shirt items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a shirt, the single shirt
  ...
  /**
   * "lists" corresponds to individual instances of the shirtList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
    /**
     * SINGLE SHIRT ACTIONS
     */
    case Actions.REQUEST_SINGLE_SHIRT: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_SHIRT: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_SHIRT_TO_MAP: {
      // add this shirt to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_SHIRT: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_SHIRT: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_SHIRT: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_SHIRT: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_SHIRT: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_DELETE_SHIRT: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_SHIRT: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_SHIRT_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: shirtList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
